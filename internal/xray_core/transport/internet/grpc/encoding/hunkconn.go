...
import (
	...
	"liuproxy_go/internal/xray_core/common/buf"
	xnet "liuproxy_go/internal/xray_core/common/net"
	"liuproxy_go/internal/xray_core/common/net/cnc"
	"liuproxy_go/internal/xray_core/common/signal/done"
)
...
func (h *HunkReaderWriter) forceFetch() error {
	...
	if err != nil {
		...
		return errors.NewError("failed to fetch hunk from gRPC tunnel").Base(err)
	}
...
func (h *HunkReaderWriter) Write(buf []byte) (int, error) {
	...
	if err != nil {
		return 0, errors.NewError("failed to send data over gRPC tunnel").Base(err)
	}
	return len(buf), nil
}
