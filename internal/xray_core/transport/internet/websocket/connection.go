...
func (c *connection) getReader() (io.Reader, error) {
	...
	// Per gorilla/websocket's doc, message types other than Binary and Text are control messages.
...
func (c *connection) Close() error {
	...
	if len(errorss) > 0 {
		return errors.NewError("failed to close connection").Base(errors.NewError(serial.Concat(errorss...)))
	}
	return nil
}
