...
func applyOutboundSocketOptions(network string, address string, fd uintptr, config *SocketConfig) error {
	if config.Interface != "" {
		...
		if err != nil {
			return errors.NewError("failed to find the interface").Base(err)
		}
		isV4 := (network == "tcp4")
		if isV4 {
			var bytes [4]byte
			...
			if err := syscall.SetsockoptInt(syscall.Handle(fd), syscall.IPPROTO_IP, IP_UNICAST_IF, int(idx)); err != nil {
				return errors.NewError("failed to set IP_UNICAST_IF").Base(err)
			}
		} else {
			if err := syscall.SetsockoptInt(syscall.Handle(fd), syscall.IPPROTO_IPV6, IPV6_UNICAST_IF, inf.Index); err != nil {
				return errors.NewError("failed to set IPV6_UNICAST_IF").Base(err)
			}
		}
	}
